...
state("Bomb Rush Cyberfunk")
{
	...
	byte wasHit : "mono-2.0-bdwgc.dll", 0x007270F8, 0x68, 0xA38, 0x30, 0x20, 0x208, 0x18;
...
split
{
	...
	||
	((current.stageID == 7 && (current.objectiveID == 11 || current.objectiveID == 13) && (current.wasHit == 1 && old.wasHit == 0)) && settings["finalAny"]))
	{
		return true;
	}
	...
	||
	((current.stageID == 7 && (current.objectiveID == 11 || current.objectiveID == 13) && (current.wasHit == 1 && old.wasHit == 0)) && settings["finalGlitchless"]))
	{
		return true;
	}
}
